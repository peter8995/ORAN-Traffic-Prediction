...
TARGET_COLUMN = "sum_granted_prbs"
...
class DataProcessor:
    def __init__(
        ...
        adj_type: str = "binary_selfloop",
        adj_corr_threshold: float = 0.3,
    ) -> None:
        ...
        self.adj_corr_threshold = adj_corr_threshold
        self.feature_names = list(BASE_FEATURE_ORDER)
        if include_target_history:
            self.feature_names.append(TARGET_COLUMN)
    ...
    def _resolve_columns(self, df: pd.DataFrame) -> Tuple[List[str], str, str | None]:
        ...
        feature_cols = [
            pick_column(FEATURE_CANDIDATES[name], name_for_error=name) for name in self.feature_names
        ]
        target_col = pick_column(FEATURE_CANDIDATES[TARGET_COLUMN], name_for_error=TARGET_COLUMN)
    ...
    def build_adjacency(self, train_records: Sequence[CsvRecord]) -> np.ndarray:
        num_nodes = len(self.feature_names)
        if self.adj_type == "binary_selfloop":
            return np.ones((num_nodes, num_nodes), dtype=np.float32)

        if self.adj_type == "binary_noselfloop":
            return np.ones((num_nodes, num_nodes), dtype=np.float32) - np.eye(
                num_nodes, dtype=np.float32
            )

        if self.adj_type == "correlation":
            train_features = np.concatenate([record.features_raw for record in train_records], axis=0)
            corr = np.corrcoef(train_features, rowvar=False)
            corr = np.nan_to_num(corr, nan=0.0, posinf=0.0, neginf=0.0)
            adjacency = (np.abs(corr) > self.adj_corr_threshold).astype(np.float32)
            np.fill_diagonal(adjacency, 1.0)
            return adjacency

        raise ValueError(f"Unsupported adj_type: {self.adj_type}")

    def _log_non_zero_ratios(self, records: Sequence[CsvRecord]) -> None:
        non_zero = np.zeros(len(self.feature_names), dtype=np.float64)
        total = 0
        for record in records:
    ...
    def prepare(
        ...
        print(
            ...
            f"num_nodes={len(self.feature_names)}"
        )
        ...
        if expanded_val_dirs is not None:
            ...
            print(
                f"[INFO] Use explicit val dirs: train={len(train_records)} "
                f"val={len(val_records)} (--val_split ignored)"
            )
        else:
        ...
        adjacency = self.build_adjacency(train_records)
